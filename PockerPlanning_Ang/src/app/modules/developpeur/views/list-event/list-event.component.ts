...
export class ListEventComponent {
    ...
    this.activityService.getActivitiesByEventId(eventId)
      .subscribe(
        (activities: Activity[]) => {
          ...
          this._router.navigate(['/list-event', eventId, 'activities']); // Redirigez vers une route dédiée aux activités avec l'ID de l'événement
        },
        (error) => {
  ...
  showParticipationForm(eventId: number): void {
    ...
    if (event && event.nbPlace === 0) {
      ...
      this._router.navigate(['/add-participation', eventId]);
    }
  }
