...
export class EventService {
  ...
  getEventsPaged(page: number, pageSize: number): Observable<any> {
    const url = `http://localhost:8080/event/paged?page=${page}&pageSize=${pageSize}`;
  ...
  getEvents(page: number, pageSize: number): Observable<any> {
    const url = `http://localhost:8080/event?page=${page}&pageSize=${pageSize}`;
    return this.httpClient.get<any>(url);
  }
  ...
  addEvent(e: Event) {
    return this.httpClient.post('http://localhost:8080/event/add', e);
  }
  public deleteEvent(id: number) {
    return this.httpClient.delete(`http://localhost:8080/event/delete/${id}`);
  }
  public updateEvent(id: number, e: Event){
    return this.httpClient.put(`http://localhost:8080/event/update/${id}`,e);
  }
  public retrieveEvent(id:number){
    return this.httpClient.get<Event>(`http://localhost:8080/event/getbyid/${id}`)
  }
  updateEventRating(id: number, rating: number): Observable<Event> {
    const url = `http://localhost:8080/event/updateRating/${id}?rating=${rating}`;
